...
```
Below is the Q-Q plot and histogram for the zombies killed by each person in the population. These data do not appear to be from a normal distribution, the  Q-Q plot points do not fall along the line and the histogram shows the distribution is skewed to the left (unimodal?). Perhaps Poisson distribution would be better? discrete count data as opposed to continuous... binomial v Poisson, probability mass functions roughly show distribution in data, can model what data looks like under different distribution, for poisson need lambda and # of sample (we have that from the samples) suggest - look at poisson distribution and actual and see if they look the same
...
```
To assess the distribution of the zombies killed data, I used dpois() to visualize the data and see if it looks the same as the histogram. In order to do this I needed lambda, or the mean # of zombies killed. 

I tried the code below but can't figure out how to check the distribution? 

zombies <- d$zombies_killed
l <- mean(d$zombies_killed)
pois_zombies <- dpois(1000, lambda = l)
barplot(pois_zombies, names.arg = 0:10, space = 0, xlab = "Zombies Killed", ylab = "Pr(X = x)", main = "Probability Mass Function")


Below is the Q-Q plot and histogram for the zombies killed by each person in the population. These data do not appear to be from a normal distribution, the  Q-Q plot points do not fall along the line and the histogram shows the distribution is skewed to the left (unimodal?). Perhaps Poisson distribution would be better? discrete count data as opposed to continuous 
...
Subset Zombies Killed - I know the zombies and education ones should be different but I am not quite sure what to do instead !!!! CI for  non-normal variables --> significance of qnorm function, other q functions to use? confidence intervals by  bootstrapping does not require previous knowledge of where your sample  comes from? bootstrapping, taking values from previous distribution, supplemental code from Module 8...
...
Subset Education 95% ci SHOULD BE POISSON CI
```{r - weight ci}
